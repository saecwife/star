...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Check if user has key on mount
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 selection:bg-gray-900 selection:text-white pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
           ...
           <button 
            ...
            className={`p-2 rounded-full transition-colors flex items-center justify-center
              ${hasUserKey ? 'text-gray-900 bg-gray-100 hover:bg-gray-200' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'}`}
            title="API Key Settings"
           >
             <span className="material-symbols-outlined">settings</span>
             {hasUserKey && <span className="absolute top-3 right-3 w-2 h-2 bg-green-500 rounded-full ring-2 ring-white"></span>}
           </button>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
        ...
        {error && (
          <div className="bg-red-50 border border-red-100 text-red-600 p-4 rounded-lg mb-8 text-center max-w-xl mx-auto text-sm flex flex-col items-center">
            ...
            <button onClick={() => setShowSettings(true)} className="text-xs font-bold underline">
              檢查 API Key 設定
            </button>
          </div>
        )}
      ...
      {showSettings && (
        <SettingsModal 
          ...
          onKeyUpdate={() => {
            setHasUserKey(!!localStorage.getItem('user_gemini_api_key'));
          }}
        />
      )}
