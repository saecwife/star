...
const getGenAIClient = () => {
  // Strict guideline compliance: Use process.env.API_KEY exclusively.
  // We do not ask the user for the key in the UI.
  const apiKey = process.env.API_KEY;
  
  if (!apiKey) {
    console.error("API Key is missing from environment variables.");
    // We throw a generic error to be handled by the UI
    throw new Error("系統設定錯誤：找不到 API Key。請確認 Cloudflare 後台設定。");
  }
